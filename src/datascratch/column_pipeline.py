...
""")
    ...
    # The
    bevel_left_start = (
        DraggableColumn.bevel_width
        + DraggableColumn.bevel_slant_width * 2
        + DraggableColumn.left_of_bevel_width
    )
    bottom_right_of_top_bevel_x = (
        bevel_left_start
        + DraggableColumn.bevel_slant_width
        + DraggableColumn.bevel_width
    )
    ...
    def paintEvent(self, event):
        ...
        if self.hovering == True:
            ...
            painter.fillRect(
                self.rect(), QColor(AppAppearance.PIPELINE_NOT_HOVER_COLOR)
            )
        ...
        holder_block = QPolygon(
            [
                ...
                QPoint(
                    ColumnsSection.bevel_left_start
                    + ColumnsSection.width_from_start_mouth_to_left_side,
        ...
        if len(lst_of_children) >= 1:
            ...
            for i in range(1, len(lst_of_children)):
                ...
                new_y = (
                    first_child.geometry().topLeft().y()
                    + i * DraggableColumn.block_height
                )
